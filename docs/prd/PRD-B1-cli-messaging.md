...
| 2026-08-14 | 开发完成：AC1-AC3 已验证（8 tests 全绿、--help 正常、无 key 失败路径明确） | 记录 B1 开发收尾 |
| 2026-08-14 | AC4 实机验证通过：`dsh-octo-bot send "你好，介绍一下你自己"` 返回完整中文回答（流式输出 + 思考过程 + session id）；期间修复：profile patch 强制官方 baseURL、CLI 凭据检查兼容凭据文件 | 真实模型调用验证成功（根因见 docs/PITFALLS.md 2.6-2.8） |
