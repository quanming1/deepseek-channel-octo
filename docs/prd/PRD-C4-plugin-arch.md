...
| 2026-08-14 | **M1 spike done** (demo/demo-plugin-spike.mjs, temp profile spike-plugin-demo): ✅ in-process `ctx.agents.create`+`followup`+`whenIdle` completes one full turn (1.7s, sane answer); ✅ **heartbeat timer keeps ticking during agent runs (0→3 ticks @500ms) — WS long connection coexists with agent execution, event loop not blocked** (architecture feasibility gate passed); ⚠️ a **second `followup` on the same live agent throws `cannot prepare session "X" while it is live`** — dsh's agent lifecycle needs the right driving pattern for consecutive in-process turns (tui supports multi-turn, so the capability exists); listed as the first M2 research item (candidates: flush-then-create/resume recovery path, or tui's inbox delivery). Side pit: `spawnSync('pnpm')` Windows ENOENT (PITFALLS 5.2) reproduced and fixed in the demo (cmd.exe /c wrapper) | FR3 in-process driving feasibility evidence; multi-turn is core for group chat — must be solved in M2, not deferred |
